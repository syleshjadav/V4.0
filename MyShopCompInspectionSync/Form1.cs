    ...
    public partial class Form1 : Form
    {
        ...
        private void UpsertInvoice()
        {
            ...
            try
            {
                ...
                if (!String.IsNullOrEmpty(cust.Tires.TireLeftFront))
                {
                    tireMinValueList.Add(new TireMinValue(cust.Tires.TireLeftFront, "LF"));
                }

                if (!String.IsNullOrEmpty(cust.Tires.TireRightFront))
                {
                    tireMinValueList.Add(new TireMinValue(cust.Tires.TireRightFront, "RF"));
                }

                if (!String.IsNullOrEmpty(cust.Tires.TireLeftRear))
                {
                    tireMinValueList.Add(new TireMinValue(cust.Tires.TireLeftRear, "LR"));
                }

                if (!String.IsNullOrEmpty(cust.Tires.TireRightRear))
                {
                    tireMinValueList.Add(new TireMinValue(cust.Tires.TireRightRear, "RR"));
                }
                ...
                if (minv != null)
                {
                    ...
                    if (md != null)
                    {
                        lowestTireReading = md.Key;
                        lowestTireValue = md.Value;
                    }
                ...
                if (t1.Rows.Count == 0) // insert a new invoice
                {
                    ...
                    sql = "INSERT INTO Invoice (UserWorkOrder,InvoiceDate,InspectDate, Lastname, Firstname,VIN,InvoiceNumber,City, State, Zip,VehicleCounty,CountyCode, Plate,VehicleMake,VehicleBody, VehicleYear," +
                      ...
                      "AirPump,CatalyticConverter,EGRvalve,EvaporativeControl,FuelInletRestrictor,PCVValve,PassInspection,StickerExpiresMonth,StickerExpiresYear,TreadLeftNR32,TreadLeft," +
                      "Labor1,LaborTotalCost,Part1,Part1Cost,Part1Qty,Station_Id,StationInspectionCharge,CurrentYear,PartsTotalCost," +
                      "LaborTotalCostInspection, PartsTotalCostInspection,TotalLaborPartsInspection,TaxAmountInspection)  " +
                ...
                else if (t1.Rows.Count == 1)
                {
                    ...
                    sql = "UPDATE Invoice SET UserWorkOrder=:UserWorkOrder,InspectDate=:InspectDate,FirstName = :FirstName, LastName=:LastName, City=:City, State=:State, Zip=:Zip," +
                                 ...
                                 " Station_Id=:Station_Id,StationInspectionCharge=:StationInspectionCharge,CurrentYear=:CurrentYear,PartsTotalCost=:PartsTotalCost," +
                                 "LaborTotalCostInspection=:LaborTotalCostInspection,PartsTotalCostInspection=:PartsTotalCostInspection,TotalLaborPartsInspection=:TotalLaborPartsInspection,TaxAmountInspection=:TaxAmountInspection" +
                             " WHERE VIN='" + cust.VIN + "'";
                ...
                using (AdsConnection cn = new AdsConnection(Connectionstring))
                {
                    using (var cmd = new AdsCommand(sql, cn))
                    {
                        ...
                        cmd.Parameters.Add("TreadLeftNR32", lowestTireValue);
                        cmd.Parameters.Add("TreadLeft", lowestTireReading);
                        ...
                        cmd.Parameters.Add("LaborTotalCost", "19.00"); // remove hardcoding
                        cmd.Parameters.Add("StationInspectionCharge", "19.00"); //  remove hardcoding

                        cmd.Parameters.Add("CurrentYear", DateTime.Now.Year.ToString()); //  remove hardcoding


                        cmd.Parameters.Add("Part1", "STATE INSPECTION  431 ANNUAL IN");
                        cmd.Parameters.Add("Part1Cost", "6.00"); // remove hardcoding
                        cmd.Parameters.Add("Part1Qty", "1");

                        cmd.Parameters.Add("PartsTotalCost", 6.00); // remove hardcoding


                        cmd.Parameters.Add("LaborTotalCostInspection", "19.00"); //  remove hardcoding
                        cmd.Parameters.Add("PartsTotalCostInspection", "6.00"); //  remove hardcoding
                        cmd.Parameters.Add("TotalLaborPartsInspection", "25.00"); //  remove hardcoding
                        cmd.Parameters.Add("TaxAmountInspection", (25 * .07).ToString()); //  remove hardcoding
