...
namespace MyShop.Core
{
    ...
    public class DealerRepoisitory
    {
        public DealerRepoisitory() { }

        public List<Dealer> GetAllDealers()
        {
            var x = new List<Dealer>();
            x.Add(new Dealer { Id = 1, DealerName = "deal 1" });
            x.Add(new Dealer { Id = 2, DealerName = "deal 2" });
            x.Add(new Dealer { Id = 3, DealerName = "deal 3" });
            x.Add(new Dealer { Id = 4, DealerName = "deal 4" });
            x.Add(new Dealer { Id = 5, DealerName = "deal 5" });
            return x;
        }

        public Dealer GetDealerById(int dealerid)
        {
            return GetAllDealers().Where(x => x.Id == dealerid).SingleOrDefault();
        }
    }
    ...
    public class CustomerVehicleRepository
    {
        ...
        public List<CustomerVehicle> GetAllCustomers()
        {
            var x = new List<CustomerVehicle>();

            x.Add(new CustomerVehicle { VehicleId = 1, VehicleMake = "Make 1", VehicleModel = "Model 1" });
            x.Add(new CustomerVehicle { VehicleId = 2, VehicleMake = "Make 2", VehicleModel = "Model 2" });
            x.Add(new CustomerVehicle { VehicleId = 3, VehicleMake = "Make 3", VehicleModel = "Model 3" });
            x.Add(new CustomerVehicle { VehicleId = 4, VehicleMake = "Make 4", VehicleModel = "Model 4" });

            return x;

        }

        public CustomerVehicle GetCustomerVehicleById(int vehicleId)
        {
            return GetAllCustomers().SingleOrDefault(x => x.VehicleId == vehicleId);
