...
namespace MoonAPNS
{
    public class PushNotification
    {
        ...
        public List<string> SendToApple(List<NotificationPayload> queue , string customMessage)
        {
            Logger.Info("Payload queue received.");
            _notifications = queue;
            if (queue.Count < 8999)
            {
                SendQueueToapple(_notifications, customMessage);
            }
            else
            {
        ...
        private void SendQueueToapple(IEnumerable<NotificationPayload> queue, string customMessage)
        {
            int i = 1000;
            foreach (var item in queue)
            {
                ...
                try
                {
                    ...
                    {
                        item.PayloadId = i;
                        byte[] payload = GeneratePayload(item, customMessage);
                       ...
                       // Logger.Info("Notification successfully sent to APNS server for Device Toekn : " + item.DeviceToken);
                        Thread.Sleep(500); //Wait to get the response from apple.
                    }
                    else
        ...
        private static byte[] GeneratePayload(NotificationPayload payload, string customMessage)
        {
            try
            {
              ...
              //  string apnMessage = payload.ToJson();
                string apnMessage = customMessage;
